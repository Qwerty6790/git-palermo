...
import { CircleUser, Search, ShoppingBasket, Bell, Home, Info, Box, Tag } from 'lucide-react';
...
const Header: React.FC = () => {
  ...
  const navLinks: NavLink[] = [
    ...
    { label: 'Бренды', href: '/brands', icon: <Tag size={20} /> },
    { label: 'Профиль', href: '/profile', icon: <CircleUser size={20} /> },
    { label: 'Корзина', href: '/cart', icon: <ShoppingBasket size={20} /> }
  ];
  ...
  return (
    <header className={clsx('fixed bg-gradient-to-r from-black to-neutral-950 w-full shadow-md z-50')}>
      <div className="container mx-auto py-8 px-5">
        <div className={clsx('transition-opacity duration-300', showTopRow ? 'opacity-100' : 'opacity-0', showTopRow ? 'h-auto' : 'h-0 overflow-hidden')}>
          <div className="flex justify-between items-center text-white mb-2">
            ...
            <div className="flex items-center  space-x-4">
              <form className="hidden md:flex items-center" onSubmit={handleSearchSubmit}> {/* Добавляем обработчик отправки */}
                <div className="relative">
                  <input
                    className=" border-white bg-transparent text-white rounded-lg py-2 pl-10 pr-3 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-300"
                    type="text"
                    placeholder="Поиск..."
        ...
        <div className="flex items-center justify-between">
            ...
            <nav ref={navRef} className="flex space-x-8 items-center">
              {navLinks.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  className="flex items-center text-white hover:text-gray-300 transition duration-300 px-4 py-2 rounded-lg hover:bg-gray-700"
                >
          ...
          <div className="flex items-center space-x-4">
            <div className="relative">
              <a href="/notifications" className="text-white hover:text-gray-300 transition duration-300 flex items-center">
                <Bell size={30} />
                <span className="absolute -top-1 -right-1 bg-red-600 text-white text-xs rounded-full px-2">0</span>
              </a>
            </div>
            <div className="relative">
